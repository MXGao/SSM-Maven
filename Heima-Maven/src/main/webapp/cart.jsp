	...
	<head>
		...
		<script type="text/javascript">
			...
			function delCartByPid(pid){
				if (confirm("确定要删除该项嘛？")){
					location.href="${pageContext.request.contextPath}/product/delCartByPid?pid="+pid;
				}
			}
			// 点击清空购物车，调用该函数
			function clearCart(){
				if (confirm("确定要清空购物车嘛？")){
				...
				<div style="margin:0 auto; margin-top:10px;width:950px;">
					...
					<table class="table table-bordered">
						<tbody>
							...
							<c:forEach items="${cart.cartItemMap}" var="cartItemMap">
								<tr class="active">
									...
									<td width="30%">
										<a target="_blank">${cartItemMap.value.product.pname}</a>
									</td>
									<td width="20%">
									...
									<td width="15%">
										<span class="subtotal">￥${cartItemMap.value.subtotal}</span>
									</td>
									<td>
										<a href="javascript:void(0);" onclick="delCartByPid(${cartItemMap.value.product.pid})" class="delete">删除</a>
									</td>
								</tr>
							</c:forEach>
						</tbody>
